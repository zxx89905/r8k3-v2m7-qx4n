...
**Goal:** Restore Songform's Spotify album search through a free Vercel serverless proxy while keeping Spotify credentials out of Git, GitHub Pages, and browser bundles.

**Architecture:** A single Vercel `GET /api/spotify` function owns Spotify Client Credentials authentication, validates browser inputs, and returns the existing Spotify JSON shapes. The Vite client keeps its current exported API but calls the proxy URL supplied through the public `VITE_SPOTIFY_PROXY_URL` GitHub Actions variable; LRCLIB stays client-side.
...
- [ ] **Step 3: Verify browser search and album selection**
